...
function part2() {
  const input: string = read_file();
  const coordinates: Coordinate[] = input.trim()
    .split('\n')
    .map(s => s.trim())
    .filter(s => !!s)
    .map(line => line.split(','))
...
function is_on_segment(a: Coordinate, b: Coordinate, p: Coordinate): boolean {
  if (a.x === b.x && p.x === a.x) {
...
function is_inside_polygon(vertices: Coordinate[], point: Coordinate): boolean {
  for (let i = 0; i < vertices.length; i++) {
    if (is_on_segment(vertices[i], vertices[(i + 1) % vertices.length], point)) return true;
  }
  let crossings = 0;
  for (let i = 0; i < vertices.length; i++) {
    const a = vertices[i];
    ...
    if (a.x === b.x) {
      ...
      if (a.x > point.x && y1 <= point.y && point.y < y2) crossings++;
    }
  }
  return crossings % 2 === 1;
}
function is_valid(rect: Rect, vertices: Coordinate[]): boolean {
  const corner_1: Coordinate = { x: rect.start.x, y: rect.end.y };
  const corner_2: Coordinate = { x: rect.end.x, y: rect.start.y };
